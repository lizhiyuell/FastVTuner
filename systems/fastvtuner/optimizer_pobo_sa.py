...
class PollingBayesianOptimization:
    def __init__(self, env, seed=1206, threshold=None, run_params='"" "" glove-100-angular', log_file="record.log", pobo_file="pobo_record.log") -> None:
        self.env = env
    ...
    def step(self,):
        ...
        sp.run(f'echo {list(self.chosen_ref_whole)} {list(self.chosen_ref_k.values())}  {list(self.delta_hv.values())} {self.worst_type_record[-1]} {self.remain_types} >> {self.pobo_file}', shell=True, stdout=sp.PIPE)

        # 使用RR方法获得一个目标索引，并由这个目标索引推荐一组尝试参数
        polling_k, new_x = self.rr_polling()

        # 实际调用engine来获取下一次使用的运行参数
