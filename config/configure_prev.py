...
# 我们在后面用到的时候再读取，这样能读到最新的
INDEX_PARAM_DICT = None

CONF_PATH = r'/root/lzy/FVDB_tuning/VDTuner/vector-db-benchmark-master/experiments/configurations/milvus-single-node.json'
ORIGIN_PATH = r'/root/lzy/FVDB_tuning/VDTuner/vector-db-benchmark-master/engine/servers/milvus-single-node/milvus.yaml.backup'
ADJUST_PATH = r'/root/lzy/FVDB_tuning/VDTuner/vector-db-benchmark-master/engine/servers/milvus-single-node/milvus.yaml'

def filter_index_rule(conf):
    global INDEX_PARAM_DICT
    if INDEX_PARAM_DICT is None:
